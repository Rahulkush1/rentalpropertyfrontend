...
export const fetchAllProperty = createAsyncThunk(
  ...
  async (filter, { rejectWithValue }) => {
    console.log(filter)
    const {keyword = "", page,sort_option, min_price, max_price , prop_type} = filter
    const config = {
      headers: {
    ...
    try {
      const resp = await axios.get(
        `${BASE_URL}/properties?keyword=${keyword}&sort_option=${sort_option}&min_price=${min_price}&max_price=${max_price}&page=${page}&prop_type=${prop_type}`,
        config
      );
